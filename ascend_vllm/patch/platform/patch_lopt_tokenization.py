...
from ascend_vllm.patch.platform import patch_envs as ascend_envs
...
from ascend_vllm.tokenization.lopt_tokenizer import (  # noqa: E402
    LoptConfig,
...
def _config_from_env() -> LoptConfig:
    return LoptConfig(
        ...
        thread_workers=ascend_envs.VLLM_ASCEND_LOPT_THREAD_WORKERS,
        min_chars=ascend_envs.VLLM_ASCEND_LOPT_MIN_CHARS,
        chunk_chars=ascend_envs.VLLM_ASCEND_LOPT_CHUNK_CHARS,
        overlap_chars=ascend_envs.VLLM_ASCEND_LOPT_OVERLAP_CHARS,
        min_match_tokens=ascend_envs.VLLM_ASCEND_LOPT_MIN_MATCH_TOKENS,
        max_retries=ascend_envs.VLLM_ASCEND_LOPT_MAX_RETRIES,
        verify=ascend_envs.VLLM_ASCEND_LOPT_VERIFY,
    )
...
def _tokenize_prompt_with_lopt(
    ...
    # When return_token_offsets is requested, defer to the original path which
    # knows how to produce prompt_token_offsets.  LoPT does not expose offsets
    # from its public encode() yet, so we avoid silently dropping them.
