...
class AscendCogVideoXBlock(nn.Module):
    r"""
    ...
    def __init__(
        ...
        self.attn1 = Attention(
            ...
            processor=AscendCogVideoXAttnProcessor2_0(),
        )
...
class AscendCogVideoXTransformer3DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, CacheMixin):
    ...
    def forward(
        ...
        if self.ofs_embedding is not None:
            ofs_embed = self.ofs_proj(ofs)
            ofs_embed = ofs_embed.to(dtype=hidden_states.dtype)
            ofs_embed = self.ofs_embedding(ofs_embed)
            emb = emb + ofs_embed
        ...
        ps_t = self.config.patch_size_t

        if ps_t is None:
            output = hidden_states.reshape(batch_size, num_frames, height // ps, width // ps, -1, ps, ps)
            output = output.permute(0, 1, 4, 2, 5, 3, 6).flatten(5, 6).flatten(3, 4)
        else:
            output = hidden_states.reshape(
                batch_size, (num_frames + ps_t - 1) // ps_t, height // ps, width // ps, -1, ps_t, ps, ps
            )
...
cogvideox_transformer_3d.CogVideoXAttnProcessor2_0 = AscendCogVideoXAttnProcessor2_0
cogvideox_transformer_3d.CogVideoXBlock = AscendCogVideoXBlock
cogvideox_transformer_3d.CogVideoXTransformer3DModel = AscendCogVideoXTransformer3DModel
