...
class AscendWanVACETransformerBlock(nn.Module):
    def __init__(
        ...
        self.attn1 = Attention(
            ...
            out_bias=True,
            processor=AscendWanAttnProcessor2_0(enhanceable=True),
        )
        ...
        self.attn2 = Attention(
            ...
            processor=AscendWanAttnProcessor2_0(enhanceable=False),
        )
    ...
    def forward(
        ...
        attn_output, _ = self.attn2(hidden_states=norm_hidden_states, encoder_hidden_states=encoder_hidden_states)
...
class AscendWanVACETransformer3DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin, CacheMixin):
    r"""
    ...
    def __init__(
        ...
        vace_layers = vace_layers if vace_layers is not None else [0, 5, 10, 15, 20, 25, 30, 35]
    ...
    def enable_parallel(self, dp_size, sp_size, kuuga_sp, enable_cp):
        # update cfg parallel
        ...
        self.parallel_manager: ParallelManager = ParallelManager(dp_size, cp_size, sp_size, kuuga_sp)
...
transformer_wan_vace.WanVACETransformerBlock = AscendWanVACETransformerBlock
transformer_wan_vace.WanVACETransformer3DModel = AscendWanVACETransformer3DModel
