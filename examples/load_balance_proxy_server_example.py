...
async def raise_for_upstream_status(response: httpx.Response) -> None:
...
def upstream_error_response(exc: Exception) -> JSONResponse:
    if isinstance(exc, UpstreamHTTPStatusError):
        headers = {"X-Upstream-Status-Code": str(exc.status_code)}
        content_type = exc.content_type or ""
        if "application/json" in content_type:
            try:
                return JSONResponse(
                    content=json.loads(exc.body.decode("utf-8")),
                    status_code=exc.status_code,
                    headers=headers,
                )
            except (UnicodeDecodeError, json.JSONDecodeError):
                pass
        body = exc.body.decode("utf-8", errors="replace") if exc.body else ""
        return JSONResponse(
            content={"error": body or f"Upstream returned HTTP {exc.status_code}"},
            status_code=exc.status_code,
            headers=headers,
        )

    return JSONResponse(
        content={"error": "Upstream request failed", "detail": str(exc)},
        status_code=502,
    )
...
async def open_stream_service_response_with_retry(
    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            stream_cm = client.stream("POST", endpoint, json=req_data, headers=headers)
            response = await stream_cm.__aenter__()
            stream_entered = True
            await raise_for_upstream_status(response)
            return response, stream_cm
        except (httpx.RequestError, UpstreamHTTPStatusError) as exc:
            if stream_entered:
...
async def handle_completions_impl(api: str, request: Request):
    ...
    try:
        ...
        try:
            initial_stream_response, initial_stream_cm = await open_stream_service_response_with_retry(
        ...
        async def generate_stream():
            ...
            nonlocal initial_stream_response
            nonlocal initial_stream_cm
            generated_token = ""
            released_kv = False
            retry_count = 0
            retry = True
            ...
            async def iter_decoder_chunks():
                nonlocal initial_stream_response
                nonlocal initial_stream_cm
                if initial_stream_response is None:
                    async for retry_chunk in stream_service_response_with_retry(
                        runtime.get_decoder_client(instance_info.decoder_key),
                        api,
                        req_data,
                        request_id=instance_info.request_id,
                ...
                response = initial_stream_response
                stream_cm = initial_stream_cm
                initial_stream_response = None
                initial_stream_cm = None
                try:
                    async for retry_chunk in response.aiter_bytes():
                        yield retry_chunk
                finally:
            ...
            try:
                while retry:
                    retry = False
                    async for chunk in iter_decoder_chunks():
        ...
        return StreamingResponse(
            ...
            status_code=initial_stream_response.status_code,
        )
    except Exception:
        import traceback
