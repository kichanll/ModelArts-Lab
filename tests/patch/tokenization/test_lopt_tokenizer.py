...
import ascend_vllm.tokenization.lopt_tokenizer as lopt_tokenizer_module
from ascend_vllm.tokenization.lopt_tokenizer import (
    ChunkEncoding,
    LoptConfig,
...
class TokenizersBackendLikeTokenizer(CharacterTokenizer):
    ...
    prepare_for_model: None = None  # type: ignore[assignment]
...
def test_position_match_does_not_scan_tokens_before_overlap(monkeypatch):
    ...
    right = ChunkEncoding(
        ...
        global_offsets=tuple((index, index + 1) for index in range(overlap_start, overlap_start + right_token_count)),
    )
    original_record = lopt_tokenizer_module._record
